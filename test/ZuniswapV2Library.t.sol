...
import "../src/AdrenalineSwapLibrary.sol";
import "../src/AdrenalineSwapFactory.sol";
import "../src/AdrenalineSwapPair.sol";
import "./mocks/ERC20Mintable.sol";

contract AdrenalineSwapLibraryTest is Test {
    ...
    AdrenalineSwapPair pair;
    AdrenalineSwapPair pair2;
    AdrenalineSwapPair pair3;
    ...
    function testGetReserves() public {
        ...
        AdrenalineSwapPair(address(pair)).mint(address(this));

        (uint256 reserve0, uint256 reserve1) = AdrenalineSwapLibrary.getReserves(
            address(factory),
            address(tokenA),
            address(tokenB)
        );
    ...
    function testQuote() public {
        uint256 amountOut = AdrenalineSwapLibrary.quote(1 ether, 1 ether, 1 ether);
        assertEq(amountOut, 1 ether);

        amountOut = AdrenalineSwapLibrary.quote(1 ether, 2 ether, 1 ether);
        assertEq(amountOut, 0.5 ether);

        amountOut = AdrenalineSwapLibrary.quote(1 ether, 1 ether, 2 ether);
        assertEq(amountOut, 2 ether);
    }

    function testPairFor() public {
        address pairAddress = AdrenalineSwapLibrary.pairFor(
            address(factory),
            address(tokenA),
            address(tokenB)
        );
    ...
    function testGetAmountOut() public {
        uint256 amountOut = AdrenalineSwapLibrary.getAmountOut(
            1000,
            1 ether,
            1.5 ether
        );
    ...
    function testGetAmountOutZeroInputAmount() public {
        ...
        AdrenalineSwapLibrary.getAmountOut(0, 1 ether, 1.5 ether);
    }

    function testGetAmountOutZeroInputReserve() public {
        vm.expectRevert(encodeError("InsufficientLiquidity()"));
        AdrenalineSwapLibrary.getAmountOut(1000, 0, 1.5 ether);
    }

    function testGetAmountOutZeroOutputReserve() public {
        vm.expectRevert(encodeError("InsufficientLiquidity()"));
        AdrenalineSwapLibrary.getAmountOut(1000, 1 ether, 0);
    }
    ...
    function testGetAmountsOut() public {
        ...
        uint256[] memory amounts = AdrenalineSwapLibrary.getAmountsOut(
            address(factory),
            0.1 ether,
            path
        );
    ...
    function testGetAmountsOutInvalidPath() public {
        ...
        AdrenalineSwapLibrary.getAmountsOut(address(factory), 0.1 ether, path);
    }

    function testGetAmountIn() public {
        uint256 amountIn = AdrenalineSwapLibrary.getAmountIn(
            1495,
            1 ether,
            1.5 ether
        );
    ...
    function testGetAmountInZeroInputAmount() public {
        vm.expectRevert(encodeError("InsufficientAmount()"));
        AdrenalineSwapLibrary.getAmountIn(0, 1 ether, 1.5 ether);
    }

    function testGetAmountInZeroInputReserve() public {
        vm.expectRevert(encodeError("InsufficientLiquidity()"));
        AdrenalineSwapLibrary.getAmountIn(1000, 0, 1.5 ether);
    }

    function testGetAmountInZeroOutputReserve() public {
        vm.expectRevert(encodeError("InsufficientLiquidity()"));
        AdrenalineSwapLibrary.getAmountIn(1000, 1 ether, 0);
    }
    ...
    function testGetAmountsIn() public {
        ...
        uint256[] memory amounts = AdrenalineSwapLibrary.getAmountsIn(
            address(factory),
            0.1 ether,
            path
        );
    ...
    function testGetAmountsInInvalidPath() public {
        ...
        vm.expectRevert(encodeError("InvalidPath()"));
        AdrenalineSwapLibrary.getAmountsIn(address(factory), 0.1 ether, path);
    }
}
